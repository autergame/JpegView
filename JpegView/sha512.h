...
//from https://github.com/noloader/SHA-Intrinsics
#pragma once
...
void sha512(const uint8_t* data, uint64_t length, uint8_t out[64])
{
	uint64_t state[8] = {
	...
	for (int i = 0; i < 8; i++)
	{
		(&out[8 * i])[0] = (state[i] >> 56) & 0xff;
		(&out[8 * i])[1] = (state[i] >> 48) & 0xff;
		(&out[8 * i])[2] = (state[i] >> 40) & 0xff;
		(&out[8 * i])[3] = (state[i] >> 32) & 0xff;
		(&out[8 * i])[4] = (state[i] >> 24) & 0xff;
		(&out[8 * i])[5] = (state[i] >> 16) & 0xff;
		(&out[8 * i])[6] = (state[i] >> 8) & 0xff;
		(&out[8 * i])[7] = state[i] & 0xff;
	}
}